...
#Aufruf:
#	exec(open("Aufrufe/funktionen/erzeugePropFunktionenErkennen.py").read())
exec(open("Funktionen/funktionen.py").read())

def main():
#Setze Variablen:
    title='Proportionale Funktionen Bestimmen'
    anfang='Bestimme die Funktionsgleichung der Form $y=M\\cdot x$.'
...
#Erzeuge Aufgaben:
    ...
    for i,ausw in enumerate(auswahl):
        a,l,d=erzeugeAfgLineareFktErkennen(art='prop',steigung=ausw,achsenlaenge=12,maxX=4,maxM=3,mitText=False)
        while d in dSamm:
            a,l,d=erzeugeAfgLineareFktErkennen(art='prop',steigung=ausw,achsenlaenge=12,maxX=4,maxM=3,mitText=False)
