...
def erzeugeStreckungsaufgaben(mitText=True,anzSpalten=[2,2],mitKoord=False):
    ...
    afgText=f'Strecke folgende{"" if n==2 else "s" } {art[n]} mit den Punkten '
    afg=[f'{afgText if mitText else ""}{", ".join([f"{x}	({punkte[x][0]}|{punkte[x][1]})" for x in list(punkte.keys())])}{" und dem Faktor" if mitText else ","} k={k}']
    if mitKoord:
