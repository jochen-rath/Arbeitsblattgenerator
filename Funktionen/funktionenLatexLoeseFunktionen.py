...
def loeseFunktion(formel='W=G*p/100',varis={'W':[16.6,'Autos'],'G':[200,'Autos'],'p':[8.2,'\\%']},ges='p',breite=12,kommaAusgabe=False):
    geg=list(varis.keys())
    geg.remove(ges)
    ...
    formelStrNw=formel
    for x in list(geg):
        print(F'{strNW(varis[x][0],True)=}')
        formelStrNw=formelStrNw.replace(x,strNW(varis[x][0],True))
...
#Steht der gesuchte Werte Links oder Rechts?
    if ges==formel.split("=")[0].replace(" ",""):
        ...
        lsg.append(F'{formelStrNw.split("=")[0]}&={str(sympy.sympify(formel.split("=")[1]))} & &§§mid~\\mbox{{Umdrehen}} \\\\'.replace(".",","))
    #    lsg.append(F'{sympy.sympify(formelStrNw.split("=")[1])}&={formelStrNw.split("=")[0]} & & \\\\')
        glLsg = loeseGleichungEinfachMitEinerVariabel(G=F'{sympy.sympify(formel.split("=")[1])} = {formel.split("=")[0]}', variable=ges, latexAusgabe=True,fracAmEnde=False)
        glLsg=glLsg[6:-3]
